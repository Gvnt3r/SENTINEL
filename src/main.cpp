...
void boot_screen() {
    ...
    tft.setTextSize(FG);
    tft.drawCentreString("SENTINEL", tftWidth / 2, 8, 1);
    tft.setTextSize(FP);
    tft.setTextColor(TFT_CYAN, TFT_BLACK);
    tft.drawCentreString("AUTOMOTIVE SECURITY", tftWidth / 2, 34, 1);
}

static void drawSentinelBootFrame(uint32_t elapsed, uint32_t duration) {
    ...
    // Redraw a complete frame so no previous sweep or progress pixels survive.
    tft.fillScreen(TFT_BLACK);
    tft.setTextSize(FG);
    tft.setTextColor(cyan, TFT_BLACK);
    tft.drawCentreString("SENTINEL", cx, 7, 1);
    tft.setTextSize(FP);
    tft.setTextColor(cyan, TFT_BLACK);
    tft.drawCentreString("CAN BUS MONITOR", cx, 32, 1);

    // Instrument-style scanner. Integer lookup avoids pulling trigonometry into boot.
    static const int8_t rayX[16] = {0, 5, 9, 12, 13, 12, 9, 5, 0, -5, -9, -12, -13, -12, -9, -5};
    static const int8_t rayY[16] = {-13, -12, -9, -5, 0, 5, 9, 12, 13, 12, 9, 5, 0, -5, -9, -12};
    uint8_t phase = (elapsed / 65) & 15;
    tft.drawCircle(cx, cy, 27, cyan);
    tft.drawCircle(cx, cy, 18, cyan);
    tft.drawFastHLine(cx - 29, cy, 59, cyan);
    tft.drawFastVLine(cx, cy - 29, 59, cyan);
    tft.drawLine(cx, cy, cx + rayX[phase] * 2, cy + rayY[phase] * 2, cyan);
    tft.fillCircle(cx + rayX[(phase + 3) & 15], cy + rayY[(phase + 3) & 15], 2, amber);
    tft.fillCircle(cx + rayX[(phase + 9) & 15] / 2, cy + rayY[(phase + 9) & 15] / 2, 2, green);

    // CAN differential pair and moving frame markers.
    const int busY = 108;
    tft.drawFastHLine(15, busY, tftWidth - 30, cyan);
    tft.drawFastHLine(15, busY + 5, tftWidth - 30, cyan);
    for (int x = 18; x < tftWidth - 15; x += 28) {
        tft.drawFastVLine(x, busY - 3, 12, cyan);
        tft.fillCircle(x, busY + 2, 2, green);
    }
    int packetX = 15 + ((elapsed / 12) % max(1, tftWidth - 38));
    tft.fillRoundRect(packetX, busY - 4, 10, 13, 2, amber);

    int barX = 13, barY = tftHeight - 17, barW = tftWidth - 26;
    uint32_t progress = min(elapsed, duration);
    int filled = (barW - 4) * progress / duration;
    tft.drawRoundRect(barX, barY, barW, 8, 3, cyan);
    if (filled > 0) tft.fillRoundRect(barX + 2, barY + 2, filled, 4, 2, cyan);
    tft.setTextSize(FP);
    tft.setTextColor(progress + 250 >= duration ? green : amber, TFT_BLACK);
    tft.drawCentreString(progress + 250 >= duration ? "SYSTEM READY" : "INITIALIZING", cx, barY - 11, 1);
}
...
void boot_screen_anim() {
    ...
    const uint32_t sentinelDuration = 3200;
    const uint32_t customDuration = 7000;
    tft.drawPixel(0, 0, 0);
    while (millis() - i < (boot_img ? customDuration : sentinelDuration)) {
        uint32_t elapsed = millis() - i;
        if (boot_img) {
            if ((elapsed > 2000) && !drawn) {
            ...
            drawSentinelBootFrame(elapsed, sentinelDuration);
            delay(32);
        }
...
void setup() {
    ...
    tft.setTextColor(TFT_CYAN, TFT_BLACK);
    tft.setTextSize(FM);
    tft.drawCentreString("SENTINEL", tft.width() / 2, tft.height() / 2 - 8, 1);
    tft.setTextSize(FP);
    tft.setTextColor(TFT_CYAN, TFT_BLACK);
    tft.drawCentreString("POWER ON", tft.width() / 2, tft.height() / 2 + 10, 1);
...
#if defined(M5STICK)
    // Fixed high-contrast Sentinel instrument palette.
    bruceConfig.bgColor = TFT_BLACK;
    bruceConfig.priColor = TFT_CYAN;
    bruceConfig.secColor = 0xFD20; // automotive amber
#endif
