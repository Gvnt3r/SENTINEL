...
void configureWebServer() {
    ...
    server->on("/api/can/ids", HTTP_GET, [](AsyncWebServerRequest *request) {
        ...
        for (const auto &item : Automotive::activity().items()) {
            ...
            body += "{\"id\":" + String(item.id) + ",\"extended\":" +
                    String(item.extended ? "true" : "false") + ",\"count\":" + String(item.count) +
                    ",\"frequencyHz\":" + String(item.frequencyHz, 2) + ",\"changedMask\":" +
                    String(item.changedMask) + ",\"last\":" + Automotive::frameToJson(item.lastFrame) + "}";
        }
    ...
    server->on("/api/can/scenarios", HTTP_GET, [](AsyncWebServerRequest *request) {
        if (!checkUserWebAuth(request)) return;
        if (!setupSdCard()) { request->send(503,"application/json","{\"error\":\"SD absente\"}"); return; }
        File root=SD.open("/Automotive/scenarios"); String body="["; bool first=true;
        if(root&&root.isDirectory()) for(File file=root.openNextFile();file;file=root.openNextFile()) { String n=file.name(); if(!file.isDirectory()&&n.endsWith(".json")){if(!first)body+=',';first=false;int slash=n.lastIndexOf('/');if(slash>=0)n=n.substring(slash+1);n.remove(n.length()-5);body+="\""+n+"\"";} file.close(); }
        if (root) root.close();
        request->send(200,"application/json",body+"]");
    });
    ...
    server->on("/api/can/scenario", HTTP_POST, [](AsyncWebServerRequest *request) {
        ...
        if(!Automotive::saveScenarioAtomic(SD,scenario,&error)){request->send(500,"application/json","{\"error\":\""+error+"\"}");return;}request->send(200,"application/json","{\"saved\":true}");
    });

    server->on("/api/can/scenario", HTTP_DELETE, [](AsyncWebServerRequest *request) {
        if (!checkUserWebAuth(request)) return;
        if(!request->hasArg("name")||!setupSdCard()){request->send(400,"application/json","{\"error\":\"name/SD requis\"}");return;}
        if(!Automotive::deleteScenario(SD,request->arg("name"))){request->send(404,"application/json","{\"error\":\"scenario introuvable\"}");return;}request->send(200,"application/json","{\"deleted\":true}");
    });
