...
void drawSubmenu(int index, std::vector<Option> &options, const char *title) {
    ...
    tft.drawRightString(String(getBattery()) + "%", tftWidth - 8, 8, 1);
    ...
    tft.setTextSize(FP);
    tft.setTextColor(bruceConfig.secColor, bruceConfig.bgColor);
    tft.drawString(heading.substring(0, 24), 12, 31, 1);
    tft.drawRightString(String(index + 1) + "/" + String(menuSize), tftWidth - 13, 31, 1);

    const int rowY[3] = {48, 72, 96};
    const int indices[3] = {
        (index - 1 + menuSize) % menuSize,
        index,
        (index + 1) % menuSize,
    };
    for (int row = 0; row < 3; row++) {
        int optionIndex = indices[row];
        bool selected = row == 1;
        uint16_t frameColor = selected ? bruceConfig.priColor : getColorVariation(bruceConfig.secColor);
        if (selected) tft.fillRoundRect(12, rowY[row], tftWidth - 28, 20, 5, bruceConfig.priColor);
        else tft.drawRoundRect(12, rowY[row], tftWidth - 28, 20, 5, frameColor);
        tft.setTextSize(FM);
        tft.setTextColor(
            options[optionIndex].enabled ? (selected ? bruceConfig.bgColor : bruceConfig.secColor)
                                         : TFT_DARKGREY,
            selected ? bruceConfig.priColor : bruceConfig.bgColor
        );
        String label = options[optionIndex].label.substring(0, 25);
        tft.drawString((selected ? "> " : "  ") + label, 18, rowY[row] + 4, 1);
    }

    int progressH = max(8, (tftHeight - 48) / menuSize);
    tft.fillRoundRect(tftWidth - 9, 48, 3, tftHeight - 55, 2, getColorVariation(bruceConfig.secColor));
    tft.fillRoundRect(
        tftWidth - 9,
        48 + index * (tftHeight - 55 - progressH) / max(1, menuSize - 1),
        3,
        progressH,
        2,
        bruceConfig.priColor
