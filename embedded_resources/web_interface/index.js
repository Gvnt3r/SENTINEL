...
async function canApi(path, options) {
  const response = await fetch(path, options);
  const text = await response.text();
  if (!response.ok) throw new Error((JSON.parse(text || '{}').error) || text || `HTTP ${response.status}`);
  return text ? JSON.parse(text) : {};
}

async function canRefresh() {
  try {
    const [status, ids, scenarios] = await Promise.all([
      canApi('/api/can/status'), canApi('/api/can/ids'), canApi('/api/can/scenarios')
    ]);
    const state = document.querySelector('#can-status');
    state.textContent = `${status.connected ? 'CAN connecté' : 'UART hors ligne'} · ${status.bitrate / 1000 || 0} kbit/s · ${status.errors} erreurs`;
    state.style.color = status.connected ? '#20df70' : '#ffb000';
    document.querySelector('#can-ids').innerHTML = ids.map(i => `<tr><td>${i.extended?'x':''}${i.id.toString(16).toUpperCase()}</td><td>${i.frequencyHz}</td><td>${i.count}</td><td>${(i.last.data||[]).map(v=>v.toString(16).padStart(2,'0')).join(' ')}</td></tr>`).join('');
    document.querySelector('#can-scenarios').innerHTML = '<option value="">Scénarios SD</option>' + scenarios.map(n=>`<option value="${n}">${n}</option>`).join('');
  } catch (e) { document.querySelector('#can-message').textContent = e.message; }
}

function canFormBody(form) { return new URLSearchParams(new FormData(form)); }

document.querySelector('#can-send-form').addEventListener('submit', async event => {
  event.preventDefault(); try { await canApi('/api/can/send',{method:'POST',body:canFormBody(event.target)}); document.querySelector('#can-message').textContent='Trame transmise'; await canRefresh(); } catch(e){document.querySelector('#can-message').textContent=e.message;}
});
document.querySelector('#can-scenario-form').addEventListener('submit', async event => {
  event.preventDefault(); try { await canApi('/api/can/scenario',{method:'POST',body:canFormBody(event.target)}); document.querySelector('#can-message').textContent='Scénario enregistré'; await canRefresh(); } catch(e){document.querySelector('#can-message').textContent=e.message;}
});
async function canDeleteScenario(){const form=document.querySelector('#can-scenario-form');try{await canApi('/api/can/scenario?name='+encodeURIComponent(form.name.value),{method:'DELETE'});document.querySelector('#can-message').textContent='Scénario supprimé';await canRefresh();}catch(e){document.querySelector('#can-message').textContent=e.message;}}
async function canLoadScenario(name){if(!name)return;try{const content=await requestGet(`/file?fs=SD&name=${encodeURIComponent('/Automotive/scenarios/'+name+'.json')}&action=edit`);const form=document.querySelector('#can-scenario-form');form.name.value=name;form.content.value=content;}catch(e){document.querySelector('#can-message').textContent=e.message;}}
canRefresh();
